(function(){
  ...
  function infoModal(){
    ...
    m.className = 'cx-top-ai-modal';
    m.innerHTML = `<div class="cx-top-ai-box"><div class="cx-top-ai-head"><h2>What does “Add updates here” do?</h2><p>This box is a plain-English CRM update tool. It sends your sentence to the AI CRM interpreter and saves the result into the same unified CRM list used by every tab.</p></div><div class="cx-top-ai-body"><ul class="cx-info-list"><li><b>Create new entries:</b> add a new lead, contact, company, deal, or task from a sentence.</li><li><b>Update existing entries:</b> match by name, company, email, phone, or lead ID and add the new information.</li><li><b>Create follow-ups:</b> turn phrases like “call Friday” or “send proposal” into next steps.</li><li><b>Keep one source of truth:</b> the update appears in Full List, Leads, Deals, Tasks, Reports, and other tabs when relevant.</li></ul><div class="cx-info-example"><b>Example:</b><br>John Henry from Henry Construction called. Phone 123-754-3808, email HenryJ@email.com. He wants a website quote and is ready for a proposal. Make the next step send a proposal.</div><div class="cx-top-ai-result">Use the quick box for short updates, or press Expand for longer notes.</div></div><div class="cx-top-ai-foot"><button id="cxTopAiInfoClose" type="button">Close</button><button id="cxTopAiInfoUseExample" class="primary" type="button">Use example</button></div></div>`;
    document.body.appendChild(m);
    m.addEventListener('click', e => { if(e.target === m) m.classList.remove('open'); });
