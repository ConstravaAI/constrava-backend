...
function cxReturn(v){const c=String(v||"/dashboard");return c.startsWith("/")&&!c.startsWith("//")&&!c.startsWith("/auth/")&&!c.startsWith("/signin")?c:"/dashboard";}
function cxPrivate(p){return p==="/dashboard"||p.startsWith("/dashboard/")||p==="/crm"||p.startsWith("/crm/")||p==="/api/dashboard"||p==="/live"||p==="/reports/latest"||p==="/sites";}
...
app.get("/welcome",(req,res)=>{if(cxReq(req))return res.redirect("/dashboard");res.sendFile(path.join(__dirname,"welcome.html"));});
