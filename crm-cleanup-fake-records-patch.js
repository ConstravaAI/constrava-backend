...
const marker = "// __crmCleanupFakeRecords_v1";
const route = `${marker}
...
async function crmCleanupDeleteFromDb(siteId, records) {
  if (!hasDb || !hasDb()) return { dbDeleted: 0, dbSkipped: "No database connection." };
  ...
  try {
    if (typeof memoryLeads !== "undefined" && Array.isArray(memoryLeads)) {
      for (let i = memoryLeads.length - 1; i >= 0; i--) {
        if (crmCleanupFakeRecordPredicate(memoryLeads[i])) {
          memoryLeads.splice(i, 1);
          removed++;
        }
      }
    }
...
app.all("/api/crm/cleanup-fake-records", async (req, res) => {
  try {
    ...
    res.json({
      ...
      matched_ids: matched.map(crmCleanupRecordId).filter(Boolean).slice(0, 100),
      memoryDeleted,
      ...dbResult,
      remaining: Array.isArray(refreshed) ? refreshed.length : null,
      message: "Removed obvious AI/test/fake CRM records only. Browser session demo records are also cleared by the dashboard on refresh."
    });
  } catch (error) {
...
if (changed) {
  ...
  console.log("[crm-cleanup-fake-records-patch] Added private fake CRM record cleanup endpoint.");
} else {
