...
function normalize(s) {
  ...
  s.users ||= [];
  s.sessions ||= [];
  for (const r of s.records) r.workspaceId ||= "demo";
  for (const e of s.events) e.workspaceId ||= "demo";
  for (const p of s.plans) p.workspaceId ||= "demo";
  for (const r of s.reports) r.workspaceId ||= "demo";
  if (!s.records.some((r) => r.workspaceId === "demo")) s.records.push(...starterRecords("demo"));
  return s;
}
...
async function save(s) {
  await fs.mkdir(path.dirname(file), { recursive: true });
  await fs.writeFile(file, `${JSON.stringify(normalize(s), null, 2)}\n`);
}
...
function html(res, markup, headers = {}) {
...
function parseCookies(req) {
  return Object.fromEntries(String(req.headers.cookie || "").split(";").map((x) => x.trim()).filter(Boolean).map((x) => {
    const i = x.indexOf("=");
    return [decodeURIComponent(i >= 0 ? x.slice(0, i) : x), decodeURIComponent(i >= 0 ? x.slice(i + 1) : "")];
  }));
}
...
function currentUser(req, s) {
  const sid = parseCookies(req).constrava_session;
  if (!sid) return null;
  const session = s.sessions.find((x) => x.id === sid);
  if (!session) return null;
  return s.users.find((u) => u.id === session.userId) || null;
}
...
function ensureUserWorkspace(s, user) {
  if (!user.workspaceId) user.workspaceId = `workspace_${user.id}`;
  if (!s.records.some((r) => r.workspaceId === user.workspaceId)) {
    s.records.push(...starterRecords(user.workspaceId));
  }
}

function requestContext(req, url, s) {
  if (url.searchParams.get("demo") === "1") return { workspaceId: "demo", demo: true, user: null };
  const user = currentUser(req, s);
  if (!user) return null;
  ensureUserWorkspace(s, user);
...
async function makePlan(s, input, workspaceId) {
  ...
  try {
    const r = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: JSON.stringify({ rawText, existingRecords: s.records.filter((x) => x.workspaceId === workspaceId).slice(-30), allowedTypes: TYPES, allowedActions: ACTIONS })
      })
    });
    ...
    return { ...fallback, ...ai, planId: id("plan"), workspaceId, source: fallback.source, aiProvider: "openai", createdAt: new Date().toISOString() };
  } catch {
    return fallback;
  }
}

function commit(s, planId, ids, workspaceId) {
  const plan = s.plans.find((p) => p.planId === planId && p.workspaceId === workspaceId);
  ...
  for (const a of plan.actions.filter((x) => selected.has(x.id) && x.actionType !== "ignore")) {
    ...
    const record = { id: id(a.recordType.toLowerCase()), workspaceId, type: a.recordType, title, status: a.recordType === "Task" || a.recordType === "Deal" ? "open" : "active", priorityScore: score(a.priorityScore), priorityReasons: a.priorityReasons || [], tags: a.tags || [], fields: a.fields || {}, relationships: a.relationships || [], sourceIds: [plan.source?.sourceId].filter(Boolean), createdAt: at, updatedAt: at, metadata: { planId, aiProvider: plan.aiProvider, reasoning: a.reasoning } };
...
function filtered(s, q = {}, workspaceId = "demo") {
  let rows = s.records.filter((r) => r.workspaceId === workspaceId);
...
function dash(s, workspaceId) {
  const rows = filtered(s, {}, workspaceId);
  ...
  return {
    metrics: { newLeads: leads.length, activeDeals: deals.length, overdueTasks: tasks.filter((t) => t.fields?.dueDate && t.fields.dueDate < new Date().toISOString().slice(0, 10)).length, conversionRate: leads.length ? Math.round((deals.length / leads.length) * 100) : 0, trafficEvents: s.events.filter((e) => e.workspaceId === workspaceId).length, revenueOpportunity: opportunity, aiCreatedRecords: rows.filter((r) => r.metadata?.aiProvider).length },
    sourcePerformance: s.sources.map((x) => ({ ...x, records: rows.filter((r) => r.sourceIds?.includes(x.id)).length, events: s.events.filter((e) => e.workspaceId === workspaceId && e.siteId === x.metadata?.siteId).length })),
    highPriority: high,
...
<html lang="en">
<head>
  <meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Constrava</title>
  <style>
    :root{--blue:#061a33;--blue2:#0d2b52;--blue3:#174675;--soft:#eaf2ff;--line:#d9e3f2;--ink:#071629;--muted:#607089;--bg:#f7fbff}
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:radial-gradient(circle at 15% 0%,#dbeaff 0,#f7fbff 34%,#ffffff 100%);color:var(--ink);font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;line-height:1.55}.wrap{width:min(1100px,calc(100% - 36px));margin:auto}
    header{position:sticky;top:0;z-index:5;background:rgba(255,255,255,.88);backdrop-filter:blur(18px);border-bottom:1px solid var(--line)}.nav{height:72px;display:flex;align-items:center;justify-content:space-between;gap:20px}.brand{display:flex;align-items:center;gap:12px;font-size:24px;font-weight:950;letter-spacing:-.04em;text-decoration:none;color:var(--blue)}.mark{width:42px;height:42px;border-radius:15px;background:linear-gradient(135deg,var(--blue),var(--blue3));color:white;display:grid;place-items:center;box-shadow:0 14px 32px rgba(6,26,51,.22)}.links{display:flex;gap:18px;align-items:center}.links a{color:#263d5c;text-decoration:none;font-weight:850;font-size:14px}
    .btn{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:13px 18px;border:1px solid var(--line);text-decoration:none;font-weight:950;background:white;color:var(--blue);box-shadow:0 12px 30px rgba(6,26,51,.08)}.btn.primary{background:var(--blue);color:white;border-color:var(--blue)}
    .hero{padding:82px 0 56px}.heroGrid{display:grid;grid-template-columns:1.05fr .95fr;gap:44px;align-items:center}.eyebrow{display:inline-flex;align-items:center;border:1px solid #bed0ea;background:var(--soft);color:var(--blue);border-radius:999px;padding:7px 12px;font-size:13px;font-weight:950}h1{font-size:clamp(44px,7vw,76px);line-height:.96;letter-spacing:-.075em;margin:18px 0 18px;color:var(--blue)}.lead{font-size:20px;color:var(--muted);max-width:650px}.actions{display:flex;gap:12px;flex-wrap:wrap;margin-top:28px}.note{font-size:14px;color:#71829b;margin-top:18px}
    .preview{background:white;border:1px solid rgba(6,26,51,.16);border-radius:34px;padding:18px;box-shadow:0 34px 90px rgba(6,26,51,.14)}.chrome{background:var(--blue);border-radius:24px;padding:14px}.dots{display:flex;gap:7px;margin-bottom:12px}.dots span{width:10px;height:10px;border-radius:999px;background:#8fa7c7}.screen{background:#f9fbff;border-radius:18px;padding:18px}.screenTop{display:flex;justify-content:space-between;gap:12px;align-items:center;border-bottom:1px solid var(--line);padding-bottom:14px;margin-bottom:14px}.badge{display:inline-flex;background:var(--soft);color:var(--blue);border:1px solid #bed0ea;border-radius:999px;padding:4px 9px;font-size:12px;font-weight:950}.metricGrid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.metric{background:white;border:1px solid var(--line);border-radius:16px;padding:12px}.metric b{display:block;font-size:24px;color:var(--blue)}.record{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;background:white;border:1px solid var(--line);border-radius:16px;padding:12px;margin-top:10px}.score{background:var(--soft);color:var(--blue);border-radius:999px;padding:5px 8px;font-weight:950}
    section{padding:62px 0}h2{font-size:clamp(32px,5vw,52px);line-height:1;letter-spacing:-.055em;margin:0 0 14px;color:var(--blue)}.sectionLead{font-size:18px;color:var(--muted);max-width:760px}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-top:24px}.card{background:rgba(255,255,255,.94);border:1px solid var(--line);border-radius:24px;padding:22px;box-shadow:0 18px 48px rgba(6,26,51,.08)}.card h3{margin:0 0 8px;font-size:22px;color:var(--blue)}.card p{margin:0;color:var(--muted)}.cta{background:linear-gradient(135deg,var(--blue),var(--blue2));color:white;border-radius:34px;padding:34px;display:flex;align-items:center;justify-content:space-between;gap:22px}.cta h2{color:white}.cta p{color:#d8e6f8;margin:0}footer{border-top:1px solid var(--line);padding:26px 0;color:#71829b;font-size:14px}
...
<body>
  ...
  <main>
    ...
    <section id="how"><div class="wrap cta"><div><h2 style="margin-bottom:10px">Try the demo or sign in.</h2><p>The public demo shows a preview workspace. Signing in opens the dashboard connected to that saved account.</p></div><div class="actions"><a class="btn primary" href="/signin">Sign in</a><a class="btn" href="/demo">Demo</a></div></div></section>
  </main>
  <footer><div class="wrap">© 2026 Constrava · <a href="/demo">Demo</a> · <a href="/signin">Sign in</a></div></footer>
</body>
</html>`;
}
...
<html lang="en">
<head>
  <meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Constrava Dashboard</title>
  <style>
    :root{--blue:#061a33;--blue2:#0d2b52;--blue3:#174675;--soft:#eaf2ff;--line:#d9e3f2;--ink:#071629;--muted:#607089;--bg:#f7fbff}
...
<body>
  <aside class="side"><div class="brand">Constrava<span>Dashboard</span></div><a class="home" href="/">← Homepage</a><nav class="nav" id="nav"></nav><div class="foot"><button id="settingsBtn">Settings</button><button id="signoutBtn">${signoutLabel}</button></div></aside>
  <main><header class="top"><div><p class="muted">${workspaceLabel}</p><h1 id="title">Dashboard</h1></div><div><input id="search" placeholder="Ask for records, tasks, leads..."> <button class="primary" id="aiBtn">AI Add</button></div></header><section class="content" id="app"></section></main>
...
<script>
const DEMO=${JSON.stringify(demo)};const API_SUFFIX=${JSON.stringify(apiSuffix)};
...
const api=function(p,o){o=o||{};return fetch(url(p),{...o,headers:{"content-type":"application/json",...(o.headers||{})}}).then(async function(r){let d=await r.json();if(r.status===401){location.href="/signin";return}if(!r.ok)throw Error(d.error||"Request failed");return d})};
...
async function signout(){if(DEMO){location.href="/";return}await fetch("/api/auth/logout",{method:"POST"});location.href="/signin"}
...
async function auth(req, res, url, s) {
  if (req.method === "GET" && url.pathname === "/api/auth/me") {
    return send(res, currentUser(req, s) ? 200 : 401, { user: publicUser(currentUser(req, s)) });
  }
  ...
  if (req.method === "POST" && (url.pathname === "/api/auth/signup" || url.pathname === "/api/auth/login")) {
    ...
    if (url.pathname === "/api/auth/signup") {
      ...
      user = { id: id("user"), email, name: clean(body.name) || email.split("@")[0], workspaceId: "", passwordSalt: pass.salt, passwordHash: pass.hash, createdAt: new Date().toISOString() };
...
async function api(req, res, url) {
  const s = await store();
  if (url.pathname.startsWith("/api/auth/")) return await auth(req, res, url, s);
  if (req.method === "GET" && url.pathname === "/api/health") return send(res, 200, { ok: true, aiConfigured: Boolean(process.env.OPENAI_API_KEY), homepage: "/", demo: "/demo", signin: "/signin", dashboard: "/dashboard" });

  const ctx = requestContext(req, url, s);
  if (!ctx) return send(res, 401, { error: "Sign in required." });

  if (req.method === "GET" && url.pathname === "/api/dashboard/summary") return send(res, 200, dash(s, ctx.workspaceId));
  if (req.method === "GET" && url.pathname === "/api/records") return send(res, 200, { records: filtered(s, Object.fromEntries(url.searchParams.entries()), ctx.workspaceId) });
  if (req.method === "GET" && url.pathname === "/api/sources") return send(res, 200, { sources: s.sources, snippet: snippet() });
  if (req.method === "GET" && url.pathname === "/api/plans") return send(res, 200, { plans: s.plans.filter((p) => p.workspaceId === ctx.workspaceId).sort((a, b) => b.createdAt.localeCompare(a.createdAt)) });
  if (req.method === "GET" && url.pathname === "/api/reports") return send(res, 200, { reports: s.reports.filter((r) => r.workspaceId === ctx.workspaceId).sort((a, b) => b.createdAt.localeCompare(a.createdAt)) });
  if (req.method === "GET" && url.pathname === "/api/analytics/events") return send(res, 200, { events: s.events.filter((e) => e.workspaceId === ctx.workspaceId).sort((a, b) => b.createdAt.localeCompare(a.createdAt)) });

  if (req.method === "POST" && url.pathname === "/api/records/plan") {
    const p = await makePlan(s, await read(req), ctx.workspaceId);
    s.plans.push(p);
    await save(s);
  ...
  if (req.method === "POST" && url.pathname === "/api/records/commit") {
    ...
    const out = commit(s, b.planId, b.actionIds, ctx.workspaceId);
    await save(s);
    return send(res, 200, out);
  }
  if (req.method === "POST" && url.pathname === "/api/analytics/events") {
    const b = await read(req);
    const e = { id: id("event"), workspaceId: ctx.workspaceId, type: clean(b.type || "custom"), siteId: clean(b.siteId || "site_demo"), sessionId: clean(b.sessionId || id("session")), sourceUrl: clean(b.sourceUrl || ""), referrer: clean(b.referrer || ""), metadata: b.metadata || {}, createdAt: new Date().toISOString() };
    s.events.push(e);
    await save(s);
  ...
  if (req.method === "POST" && url.pathname === "/api/sources/form") {
    const b = await read(req);
    const p = await makePlan(s, { kind: "website_form", sourceId: "source_website", rawText: b.rawText || JSON.stringify(b.fields || b) }, ctx.workspaceId);
    s.plans.push(p);
    await save(s);
  ...
  if (req.method === "POST" && url.pathname === "/api/search/natural") {
    ...
    return send(res, 200, { plan: { q, explanation: "Converted plain English into safe filters." }, records: filtered(s, { q, type: /deal|quote/.test(q) ? "Deal" : /task|follow/.test(q) ? "Task" : "" }, ctx.workspaceId) });
  }
  if (req.method === "POST" && url.pathname === "/api/reports/generate") {
    const d = dash(s, ctx.workspaceId);
    const content = { title: "Business Activity Report", factualSummary: [`${d.metrics.newLeads} lead/contact records are tracked.`, `${d.metrics.activeDeals} active deals represent $${d.metrics.revenueOpportunity.toLocaleString()} in opportunity.`, `${d.metrics.trafficEvents} analytics events have been captured.`], recommendations: d.recommendedActions.map((x) => `${x.title}: ${x.reason}`) };
    const r = { id: id("report"), workspaceId: ctx.workspaceId, title: content.title, content, createdAt: new Date().toISOString() };
    s.reports.push(r);
    await save(s);
...
http.createServer(async (req, res) => {
  try {
    ...
    if (["/dashboard", "/app"].includes(url.pathname)) {
      const user = currentUser(req, s);
      if (!user) return html(res, signInPage());
      ensureUserWorkspace(s, user);
      await save(s);
