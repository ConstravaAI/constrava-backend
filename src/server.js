...
function nav(){nav.innerHTML=views.map(v=>'<button class="'+(S.view==v?'active':'')+'" onclick="view(\\''+v+'\\')">'+v[0].toUpperCase()+v.slice(1)+'</button>').join("")}function view(v){S.view=v;render()}async function refresh(v=S.view){await load();S.view=v;render()}
