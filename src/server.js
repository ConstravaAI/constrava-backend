...
async function api(req, res, url, route) {
  ...
  if (route === "/api/projects" || route.startsWith("/api/projects/")) {
    ...
    if (memberAccessMatch) {
      ...
      if (membership.role === "owner") return send(res, 409, { error: "The project ownerâ€™s access cannot be changed." });
      if (req.method === "PATCH") {
