...
async function makeLocalPlan(input, workspaceId, storeData = null) {
  ...
  if (/follow|call|email|schedule|meeting|tomorrow|today|next\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)|in\s+(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+(?:days?|weeks?)/i.test(rawText)) plan.actions.push(action("create_task", "Task", { title: fields.companyName ? `Follow up with ${fields.companyName}` : "Follow up on new record", taskType: /call|meeting|schedule/i.test(rawText) ? "call" : "email", ...(resolvedDueDate ? { dueDate: resolvedDueDate, dueDateSource: dateContext.resolvedDates[0].phrase } : {}) }, priorityData, ["needs follow-up", ...tags], resolvedDueDate ? `Next-action language found; ${dateContext.resolvedDates[0].phrase} resolves to ${resolvedDueDate} in ${dateContext.timeZone}.` : "Next-action language found."));
  plan.actions.push(action("attach_note", "Note", { title: plan.actions.length ? "Source note" : fields.request || "Business note", body: rawText }, priorityData, tags, plan.actions.length ? "Keep the original context attached." : "No stronger entity or action signal was found, so preserve the information as a reviewable note."));
  return plan;
}
