...
function analyticsMetricRefreshButton(key){return '<button class="analyticsMetricRefresh secondary" type="button" style="min-height:28px;border-radius:7px;border:1px solid #cbd8ea;background:#f8fbff;color:##1c4687;font-size:11px;font-weight:850;padding:4px 7px;box-shadow:none" onclick="analyticsRefreshMetric(&quot;'+key+'&quot;)">'+(S.analyticsMetricRefreshing===key?'Updating':'Refresh')+'</button>'}
...
function analyticsDedicatedMetrics(){return analyticsMetricTray(analyticsUniqueSessionsMetric()+analyticsTotalEventsMetric()+analyticsPageViewsMetric()+analyticsFormSubmitsMetric()+analyticsConversionRateMetric()+analyticsActivePagesMetric())}`;
    ...
    const commandCenterBlueGradient = `
          radial-gradient(circle at 12% 0%, rgba(18,58,99,.32), transparent 34%),
          radial-gradient(circle at 88% 18%, rgba(34,86,142,.24), transparent 30%),
          linear-gradient(135deg, #020817 0%, #071a33 56%, #123a63 100%) !important`;

    const analyticsTextStyles = `
      /* analytics-command-center-blue-gradient-v2 */
      .analyticsStickyCommandCenter {
        ...
        background:${commandCenterBlueGradient};
      }
    ...
    if (!generated.includes("analytics-command-center-blue-gradient-v2")) {
