...
source = source.replace("let S={tab:\"analytics\",crmView:\"overview\",records:[],plans:[],plan:null,summary:null,sources:[],events:[],reports:[],snippet:\"\"};", "let S={tab:\"analytics\",crmView:\"overview\",records:[],plans:[],plan:null,summary:null,sources:[],events:[],reports:[],snippet:\"\",settings:{},settingsLoaded:false};");
source = source.replace("async function load(){let out=await Promise.all([api('/api/dashboard/summary'),api('/api/records'),api('/api/sources'),api('/api/plans'),api('/api/reports'),api('/api/analytics/events')]);S.summary=out[0];S.records=out[1].records;S.sources=out[2].sources;S.snippet=out[2].snippet;S.plans=out[3].plans;S.reports=out[4].reports;S.events=out[5].events;syncNotifications()}", "async function load(){let out=await Promise.all([api('/api/dashboard/summary'),api('/api/records'),api('/api/sources'),api('/api/plans'),api('/api/reports'),api('/api/analytics/events'),api('/api/settings')]);S.summary=out[0];S.records=out[1].records;S.sources=out[2].sources;S.snippet=out[2].snippet;S.plans=out[3].plans;S.reports=out[4].reports;S.events=out[5].events;S.settings=(out[6]&&out[6].settings)||{};if(!S.settingsLoaded){S.settingsLoaded=true;if(S.settings.defaultCrmView)S.crmView=S.settings.defaultCrmView}syncNotifications()}");
